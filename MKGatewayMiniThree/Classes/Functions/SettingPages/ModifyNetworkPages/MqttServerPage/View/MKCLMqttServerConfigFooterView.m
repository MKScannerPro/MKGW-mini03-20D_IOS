//
//  MKCLMqttServerConfigFooterView.m
...
@implementation MKCLMqttServerConfigFooterViewModel
@end
...
@interface MKCLMqttServerConfigFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKCLMqttServerSSLViewDelegate,
MKCLMqttServerSettingViewDelegate,
MKCLMqttServerLwtViewDelegate>
...
@property (nonatomic, strong)MKCLMqttServerSSLView *sslView;

@property (nonatomic, strong)MKCLMqttServerLwtView *lwtView;

@property (nonatomic, strong)MKCLMqttServerSettingView *settingView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKCLMqttServerConfigFooterView
...
#pragma mark - MKCLMqttServerSSLViewDelegate
- (void)cl_mqtt_sslParams_modifyDevice_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(cl_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
...
- (void)cl_mqtt_sslParams_modifyDevice_certificateChanged:(NSInteger)certificate {
...
/// @param value value
- (void)cl_mqtt_sslParams_modifyDevice_textFieldValueChanged:(NSInteger)index value:(NSString *)value {
    ...
    if ([self.delegate respondsToSelector:@selector(cl_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate cl_mqtt_serverForDevice_textFieldValueChanged:value textID:tempIndex];
    }
}

#pragma mark - MKCLMqttServerSettingViewDelegate
...
#pragma mark - MKCLMqttServerLwtViewDelegate
...
#pragma mark - setter
- (void)setDataModel:(MKCLMqttServerConfigFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCLMqttServerConfigFooterViewModel.class]) {
        return;
    }
    ...
    MKCLMqttServerSSLViewModel *sslModel = [[MKCLMqttServerSSLViewModel alloc] init];
    ...
    MKCLMqttServerLwtViewModel *lwtModel = [[MKCLMqttServerLwtViewModel alloc] init];
...
- (MKCLMqttServerSSLView *)sslView {
    if (!_sslView) {
        _sslView = [[MKCLMqttServerSSLView alloc] init];
...
- (MKCLMqttServerLwtView *)lwtView {
    if (!_lwtView) {
        _lwtView = [[MKCLMqttServerLwtView alloc] init];
...
- (MKCLMqttServerSettingView *)settingView {
    if (!_settingView) {
        _settingView = [[MKCLMqttServerSettingView alloc] init];
