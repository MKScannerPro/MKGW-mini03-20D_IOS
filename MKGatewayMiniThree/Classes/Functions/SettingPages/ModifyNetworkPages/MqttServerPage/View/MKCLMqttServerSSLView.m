...
@implementation MKCLMqttServerSSLViewModel
@end

@interface MKCLMqttServerSSLView ()<MKCLMqttServerSSLTextFieldDelegate>
...
@property (nonatomic, strong)MKCLMqttServerSSLTextField *caFileView;

@property (nonatomic, strong)MKCLMqttServerSSLTextField *clientKeyView;

@property (nonatomic, strong)MKCLMqttServerSSLTextField *clientCertView;

@end

@implementation MKCLMqttServerSSLView
...
#pragma mark - MKCLMqttServerSSLTextFieldDelegate
/// textField内容发送改变时的回调事件
/// @param index 当前cell所在的index
/// @param value 当前textField的值
- (void)cl_modifyServerSSLTextFieldValueChanged:(NSInteger)index textValue:(NSString *)value {
    if ([self.delegate respondsToSelector:@selector(cl_mqtt_sslParams_modifyDevice_textFieldValueChanged:value:)]) {
        [self.delegate cl_mqtt_sslParams_modifyDevice_textFieldValueChanged:index value:value];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(cl_mqtt_sslParams_modifyDevice_sslStatusChanged:)]) {
        [self.delegate cl_mqtt_sslParams_modifyDevice_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(cl_mqtt_sslParams_modifyDevice_certificateChanged:)]) {
            [self.delegate cl_mqtt_sslParams_modifyDevice_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCLMqttServerSSLViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCLMqttServerSSLViewModel.class]) {
        return;
    }
    ...
    MKCLMqttServerSSLTextFieldModel *caModel = [[MKCLMqttServerSSLTextFieldModel alloc] init];
    caModel.index = 0;
    ...
    MKCLMqttServerSSLTextFieldModel *clientKeyModel = [[MKCLMqttServerSSLTextFieldModel alloc] init];
    ...
    MKCLMqttServerSSLTextFieldModel *clientModel = [[MKCLMqttServerSSLTextFieldModel alloc] init];
    clientModel.index = 2;
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKGatewayMiniThree", @"MKCLMqttServerSSLView", @"cl_switchSelectedIcon.png") : LOADICON(@"MKGatewayMiniThree", @"MKCLMqttServerSSLView", @"cl_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLMqttServerSSLView", @"cl_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (MKCLMqttServerSSLTextField *)caFileView {
    if (!_caFileView) {
        _caFileView = [[MKCLMqttServerSSLTextField alloc] init];
        _caFileView.delegate = self;
    }
    return _caFileView;
}

- (MKCLMqttServerSSLTextField *)clientKeyView {
    if (!_clientKeyView) {
        _clientKeyView = [[MKCLMqttServerSSLTextField alloc] init];
        _clientKeyView.delegate = self;
    }
    return _clientKeyView;
}

- (MKCLMqttServerSSLTextField *)clientCertView {
    if (!_clientCertView) {
        _clientCertView = [[MKCLMqttServerSSLTextField alloc] init];
