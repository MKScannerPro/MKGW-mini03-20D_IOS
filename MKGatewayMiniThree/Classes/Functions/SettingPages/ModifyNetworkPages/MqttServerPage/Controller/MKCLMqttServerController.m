...
@interface MKCLMqttServerController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKCLMqttServerConfigFooterViewDelegate,
...
@property (nonatomic, strong)MKCLMqttServerModel *dataModel;

@property (nonatomic, strong)MKCLMqttServerConfigFooterView *sslParamsView;

@property (nonatomic, strong)MKCLMqttServerConfigFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKCLMqttServerController

- (void)dealloc {
    NSLog(@"MKCLMqttServerController销毁");
}
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveUpdateCerts:)
                                                 name:MKCLReceiveDeviceUpdateMqttCertsResultNotification
                                               object:nil];
}
...
#pragma mark - MKCLMqttServerConfigFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
...
- (void)cl_selectedServerParams:(NSString *)fileName {
    [MKCLExcelDataManager parseDeviceExcel:fileName
        ...
        MKCLMqttServerModel *model = [MKCLMqttServerModel mk_modelWithJSON:returnData];
...
- (void)clearAllParams {
    MKCLMqttServerModel *model = [[MKCLMqttServerModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLMqttServerController", @"cl_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCLMqttServerModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCLMqttServerModel alloc] init];
    }
    return _dataModel;
}
...
- (MKCLMqttServerConfigFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKCLMqttServerConfigFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKCLMqttServerConfigFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKCLMqttServerConfigFooterViewModel alloc] init];
    }
