//
//  MKCLMqttWifiSettingsBandCell.m
...
@implementation MKCLMqttWifiSettingsBandCellModel
@end

@interface MKCLMqttWifiSettingsBandCell ()
...
@implementation MKCLMqttWifiSettingsBandCell

+ (MKCLMqttWifiSettingsBandCell *)initCellWithTableView:(UITableView *)tableView {
    MKCLMqttWifiSettingsBandCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCLMqttWifiSettingsBandCellIdenty"];
    if (!cell) {
        cell = [[MKCLMqttWifiSettingsBandCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCLMqttWifiSettingsBandCellIdenty"];
    }
    return cell;
}
...
- (void)selectedButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.dataList selectedRow:row block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(cl_mqttWifiSettingsBandCell_countryChanged:)]) {
            [self.delegate cl_mqttWifiSettingsBandCell_countryChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCLMqttWifiSettingsBandCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCLMqttWifiSettingsBandCellModel.class] || _dataModel.country >= self.dataList.count) {
        return;
    }
