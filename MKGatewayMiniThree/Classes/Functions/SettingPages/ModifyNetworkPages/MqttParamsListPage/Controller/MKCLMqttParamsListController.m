//
//  MKCLMqttParamsListController.m
...
@interface MKCLMqttParamsListController ()<UITableViewDelegate, UITableViewDataSource>
...
@property (nonatomic, strong)MKCLMqttParamsModel *dataModel;

@end

@implementation MKCLMqttParamsListController

- (void)dealloc {
    NSLog(@"MKCLMqttParamsListController销毁");
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //WIFI Settings
        MKCLMqttWifiSettingsController *vc = [[MKCLMqttWifiSettingsController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        //MQTT Settings
        MKCLMqttServerController *vc = [[MKCLMqttServerController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)updateLocaDeviceData {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKCLDeviceDatabaseManager updateClientID:self.dataModel.clientID subscribedTopic:self.dataModel.subscribeTopic publishedTopic:self.dataModel.publishTopic lwtStatus:self.dataModel.lwtStatus lwtTopic:self.dataModel.lwtTopic macAddress:[MKCLDeviceModeManager shared].macAddress sucBlock:^{
        [[MKHudManager share] hide];
        [self.view showCentralToast:@"Setup succeed!"];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_cl_deviceModifyMQTTServerSuccessNotification"
                                                            object:nil
                                                          userInfo:@{
            @"macAddress":SafeStr([MKCLDeviceModeManager shared].macAddress),
...
- (MKCLMqttParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCLMqttParamsModel alloc] init];
    }
    return _dataModel;
}
