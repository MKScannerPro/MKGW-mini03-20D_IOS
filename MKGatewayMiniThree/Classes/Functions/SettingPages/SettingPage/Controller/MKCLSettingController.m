...
@interface MKCLSettingController ()<UITableViewDelegate,
...
@implementation MKCLSettingController

- (void)dealloc {
    NSLog(@"MKCLSettingController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //Indicator settings
        MKCLIndicatorSettingsController *vc = [[MKCLIndicatorSettingsController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        //Network status report interval
        MKCLNetworkStatusController *vc = [[MKCLNetworkStatusController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 2) {
        //Reconnect timeout
        MKCLReconnectTimeController *vc = [[MKCLReconnectTimeController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 3) {
        //Communicate timeout
        MKCLCommunicateController *vc = [[MKCLCommunicateController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 4) {
        //System time
        MKCLSystemTimeController *vc = [[MKCLSystemTimeController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 5) {
        //Advertise iBeacon
        MKCLAdvBeaconController *vc = [[MKCLAdvBeaconController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 6) {
        //Reset device by button
        MKCLResetByButtonController *vc = [[MKCLResetByButtonController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 7) {
        //Data report timout
        MKCLDataReportController *vc = [[MKCLDataReportController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 0) {
        //OTA
        MKCLOTAController *vc = [[MKCLOTAController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 1) {
        //Modify network settings
        MKCLMqttParamsListController *vc = [[MKCLMqttParamsListController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 0) {
        //Device information
        MKCLDeviceInfoController *vc = [[MKCLDeviceInfoController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)rootButtonPressed {
    ...
    [alertView showAlertWithTitle:@"Reboot Device" message:msg notificationName:@"mk_cl_needDismissAlert"];
}
...
- (void)resetButtonPressed {
    ...
    [alertView addAction:cancelAction];
    [alertView addAction:confirmAction];
    [alertView showAlertWithTitle:@"Reset Device" message:msg notificationName:@"mk_cl_needDismissAlert"];
}
...
- (void)configLocalName{
    ...
    self.localNameAsciiStr = SafeStr([MKCLDeviceModeManager shared].deviceName);
    MKAlertViewTextField *textField = [[MKAlertViewTextField alloc] initWithTextValue:SafeStr([MKCLDeviceModeManager shared].deviceName)
    ...
    [alertView showAlertWithTitle:@"Edit Local Name" message:msg notificationName:@"mk_cl_needDismissAlert"];
}
...
- (void)saveDeviceLocalName {
    ...
    [MKCLDeviceDatabaseManager updateLocalName:self.localNameAsciiStr
                                    macAddress:[MKCLDeviceModeManager shared].macAddress
                                      sucBlock:^{
        [[MKHudManager share] hide];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_cl_deviceNameChangedNotification"
                                                            object:nil
                                                          userInfo:@{
                                                              @"macAddress":[MKCLDeviceModeManager shared].macAddress,
...
- (void)resetDevice {
    ...
    [MKCLMQTTInterface cl_resetDeviceWithMacAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        [self removeDevice];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
...
- (void)removeDevice {
    ...
    [MKCLDeviceDatabaseManager deleteDeviceWithMacAddress:[MKCLDeviceModeManager shared].macAddress sucBlock:^{
        [[MKHudManager share] hide];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_cl_deleteDeviceNotification"
                                                            object:nil
                                                          userInfo:@{@"macAddress":[MKCLDeviceModeManager shared].macAddress}];
...
- (void)rebootDevice {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKCLMQTTInterface cl_rebootDeviceWithMacAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLSettingController", @"cl_editIcon.png") forState:UIControlStateNormal];
