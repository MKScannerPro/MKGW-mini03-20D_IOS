...
@interface MKCLSystemTimeController ()<UITableViewDelegate,
UITableViewDataSource,
MKCLSystemTimeCellDelegate,
...
@implementation MKCLSystemTimeController

- (void)dealloc {
    NSLog(@"MKCLSystemTimeController销毁");
    if (self.readTimer) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0)  {
        //Sync Time From NTP
        MKCLNTPServerController *vc = [[MKCLNTPServerController alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKCLSystemTimeCell *cell = [MKCLSystemTimeCell initCellWithTableView:tableView];
...
#pragma mark - MKCLSystemTimeCellDelegate
...
- (void)readDataFromServer {
    ...
    [MKCLMQTTInterface cl_readDeviceUTCTimeWithMacAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)syncTimeZoneToDevice:(NSInteger)timeZone {
    ...
    [MKCLMQTTInterface cl_configDeviceTimeZone:timeZone - 24 timestamp:[[NSDate date] timeIntervalSince1970] macAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)loadSection1Datas {
    MKCLSystemTimeCellModel *cellModel = [[MKCLSystemTimeCellModel alloc] init];
    cellModel.index = 0;
