...
@interface MKCLNTPServerController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCLNTPServerModel *dataModel;

@end

@implementation MKCLNTPServerController

- (void)dealloc {
    NSLog(@"MKCLNTPServerController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLNTPServerController", @"cl_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCLNTPServerModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCLNTPServerModel alloc] init];
    }
    return _dataModel;
}
