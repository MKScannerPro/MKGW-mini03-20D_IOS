...
@interface MKCLReconnectTimeModel ()
...
@implementation MKCLReconnectTimeModel
...
- (BOOL)readReconnectTimeout {
    ...
    [MKCLMQTTInterface cl_readNetworkReconnectTimeoutWithMacAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configReconnectTimeout {
    __block BOOL success = NO;
    [MKCLMQTTInterface cl_configReconnectTimeout:[self.timeout integerValue] macAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
