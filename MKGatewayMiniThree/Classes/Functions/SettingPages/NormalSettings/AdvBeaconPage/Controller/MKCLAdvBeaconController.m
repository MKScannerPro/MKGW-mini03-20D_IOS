...
@interface MKCLAdvBeaconController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCLAdvBeaconModel *dataModel;

@end

@implementation MKCLAdvBeaconController

- (void)dealloc {
    NSLog(@"MKCLAdvBeaconController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLAdvBeaconController", @"cl_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCLAdvBeaconModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCLAdvBeaconModel alloc] init];
    }
    return _dataModel;
}
