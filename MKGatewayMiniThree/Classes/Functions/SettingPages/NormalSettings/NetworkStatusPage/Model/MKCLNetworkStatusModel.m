...
@interface MKCLNetworkStatusModel ()
...
@implementation MKCLNetworkStatusModel
...
- (BOOL)readReportInterval {
    ...
    [MKCLMQTTInterface cl_readNetworkStatusReportIntervalWithMacAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configReportInterval {
    __block BOOL success = NO;
    [MKCLMQTTInterface cl_configNetworkStatusReportInterval:[self.interval integerValue] macAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
