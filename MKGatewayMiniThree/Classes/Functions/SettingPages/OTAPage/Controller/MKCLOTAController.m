...
@interface MKCLOTAController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCLOTAPageModel *dataModel;

@end

@implementation MKCLOTAController

- (void)dealloc {
    NSLog(@"MKCLOTAController销毁");
        ...
        if (self.dataModel.otaType == 0) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveOTAResult:)
                                                         name:MKCLReceiveDeviceOTAResultNotification
                                                       object:nil];
        }else if (self.dataModel.otaType == 1) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveOTAResult:)
                                                         name:MKCLReceiveDeviceNpcOTAResultNotification
                                                       object:nil];
        }
...
- (MKCLOTAPageModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCLOTAPageModel alloc] init];
    }
    return _dataModel;
}
