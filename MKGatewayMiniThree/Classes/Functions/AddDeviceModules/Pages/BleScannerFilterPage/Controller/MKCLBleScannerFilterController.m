//
//  MKCLBleScannerFilterController.m
...
@interface MKCLBleScannerFilterController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCLBleScannerFilterModel *dataModel;

@end

@implementation MKCLBleScannerFilterController

- (void)dealloc {
    NSLog(@"MKCLBleScannerFilterController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLBleScannerFilterController", @"cl_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCLBleScannerFilterModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCLBleScannerFilterModel alloc] init];
    }
    return _dataModel;
}
