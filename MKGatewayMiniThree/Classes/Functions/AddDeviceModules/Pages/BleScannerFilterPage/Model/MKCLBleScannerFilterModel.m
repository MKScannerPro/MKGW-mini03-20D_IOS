...
@interface MKCLBleScannerFilterModel ()
...
@implementation MKCLBleScannerFilterModel
...
- (BOOL)readFilterRssi {
    ...
    [MKCLInterface cl_readRssiFilterValueWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterRssi {
    __block BOOL success = NO;
    [MKCLInterface cl_configRssiFilterValue:self.rssi sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readMacAddress {
    __block BOOL success = NO;
    [MKCLInterface cl_readFilterMACAddressListWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configMacAddress {
    ...
    [MKCLInterface cl_configFilterMACAddressList:macList
                             sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    }
...
- (BOOL)readAdvName {
    __block BOOL success = NO;
    [MKCLInterface cl_readFilterAdvNameListWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configAdvName {
    ...
    [MKCLInterface cl_configFilterAdvNameList:nameList
                             sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    }
...
- (BOOL)configRelationShip {
    __block BOOL success = NO;
    [MKCLInterface cl_configFilterRelationship:7
                             sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    }
