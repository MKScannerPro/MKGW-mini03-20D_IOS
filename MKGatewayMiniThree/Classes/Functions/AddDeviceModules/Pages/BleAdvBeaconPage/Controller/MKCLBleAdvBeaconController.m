...
@interface MKCLBleAdvBeaconController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCLBleAdvBeaconModel *dataModel;

@end

@implementation MKCLBleAdvBeaconController

- (void)dealloc {
    NSLog(@"MKCLBleAdvBeaconController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLBleAdvBeaconController", @"cl_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCLBleAdvBeaconModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCLBleAdvBeaconModel alloc] init];
    }
    return _dataModel;
}
