...
@interface MKCLBleNTPTimezoneModel ()
...
@implementation MKCLBleNTPTimezoneModel
...
- (BOOL)readNTPHost {
    ...
    [MKCLInterface cl_readNTPServerHostWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configNTPHost {
    __block BOOL success = NO;
    [MKCLInterface cl_configNTPServerHost:self.ntpHost sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readTimezone {
    __block BOOL success = NO;
    [MKCLInterface cl_readTimeZoneWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configTimezone {
    __block BOOL success = NO;
    [MKCLInterface cl_configTimeZone:(self.timeZone - 24) sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
