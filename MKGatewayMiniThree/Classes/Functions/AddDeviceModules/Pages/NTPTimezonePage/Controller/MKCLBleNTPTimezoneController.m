//
//  MKCLBleNTPTimezoneController.m
...
@interface MKCLBleNTPTimezoneController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCLBleNTPTimezoneModel *dataModel;

@property (nonatomic, strong)NSMutableArray *timezoneList;

@end

@implementation MKCLBleNTPTimezoneController

- (void)dealloc {
    NSLog(@"MKCLBleNTPTimezoneController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLBleNTPTimezoneController", @"cl_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCLBleNTPTimezoneModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCLBleNTPTimezoneModel alloc] init];
    }
    return _dataModel;
}
