//
//  MKCLServerForDeviceController.m
...
@interface MKCLServerForDeviceController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKCLServerConfigDeviceFooterViewDelegate,
MKCAFileSelectControllerDelegate,
MFMailComposeViewControllerDelegate,
MKCLImportServerControllerDelegate>
...
@property (nonatomic, strong)MKCLServerForDeviceModel *dataModel;

@property (nonatomic, strong)MKCLServerConfigDeviceFooterView *sslParamsView;

@property (nonatomic, strong)MKCLServerConfigDeviceFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKCLServerForDeviceController

- (void)dealloc {
    NSLog(@"MKCLServerForDeviceController销毁");
}
...
#pragma mark - MKCLServerConfigDeviceFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl    2:lwtStatus  3:lwtRetain
- (void)cl_mqtt_serverForDevice_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)cl_mqtt_serverForDevice_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keepAlive
...
- (void)cl_mqtt_serverForDevice_qosChanged:(NSInteger)qos qosID:(NSInteger)qosID{
    if (qosID == 0) {
        //qos
...
- (void)cl_mqtt_serverForDevice_certificateChanged:(NSInteger)certificate {
...
- (void)cl_mqtt_serverForDevice_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFile
...
- (void)cl_mqtt_serverForDevice_bottomButtonPressed:(NSInteger)index {
    if (index == 0) {
        //Export Demo File
    ...
    if (index == 1) {
        //Import Config File
        MKCLImportServerController *vc = [[MKCLImportServerController alloc] init];
        vc.delegate = self;
        [self.navigationController pushViewController:vc animated:YES];
    }
    if (index == 2) {
...
#pragma mark - MKCLImportServerControllerDelegate
- (void)cl_selectedServerParams:(NSString *)fileName {
    [MKCLExcelDataManager parseDeviceExcel:fileName
                                   sucBlock:^(NSDictionary * _Nonnull returnData) {
        MKCLServerForDeviceModel *model = [MKCLServerForDeviceModel mk_modelWithJSON:returnData];
...
- (void)saveDataToDevice {
    ...
    [self.dataModel configDataWithSucBlock:^{
        ...
        [MKCLDeviceMQTTParamsModel shared].mqttConfig = YES;
        [MKCLDeviceMQTTParamsModel shared].deviceModel.clientID = self.dataModel.clientID;
        [MKCLDeviceMQTTParamsModel shared].deviceModel.deviceName = self.dataModel.deviceName;
        [MKCLDeviceMQTTParamsModel shared].deviceModel.subscribedTopic = self.dataModel.subscribeTopic;
        [MKCLDeviceMQTTParamsModel shared].deviceModel.publishedTopic = self.dataModel.publishTopic;
        [MKCLDeviceMQTTParamsModel shared].deviceModel.macAddress = self.dataModel.macAddress;
        [MKCLDeviceMQTTParamsModel shared].deviceModel.lwtStatus = self.dataModel.lwtStatus;
        [MKCLDeviceMQTTParamsModel shared].deviceModel.lwtTopic = self.dataModel.lwtTopic;
    } failedBlock:^(NSError * _Nonnull error) {
        @strongify(self);
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
        [MKCLDeviceMQTTParamsModel shared].deviceModel.clientID = @"";
        [MKCLDeviceMQTTParamsModel shared].deviceModel.deviceName = @"";
        [MKCLDeviceMQTTParamsModel shared].deviceModel.subscribedTopic = @"";
        [MKCLDeviceMQTTParamsModel shared].deviceModel.publishedTopic = @"";
        [MKCLDeviceMQTTParamsModel shared].deviceModel.macAddress = @"";
        [MKCLDeviceMQTTParamsModel shared].deviceModel.lwtStatus = NO;
        [MKCLDeviceMQTTParamsModel shared].deviceModel.lwtTopic = @"";
    }];
}
...
//    }
    ...
    [MKCLExcelDataManager exportDeviceExcel:self.dataModel
                                    sucBlock:^{
        [[MKHudManager share] hide];
        [self sharedExcel];
    }
...
- (void)clearAllParams {
    MKCLServerForDeviceModel *emptyModel = [[MKCLServerForDeviceModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLServerForDeviceController", @"cl_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCLServerForDeviceModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCLServerForDeviceModel alloc] init];
    }
    return _dataModel;
}
...
- (MKCLServerConfigDeviceFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKCLServerConfigDeviceFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKCLServerConfigDeviceFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKCLServerConfigDeviceFooterViewModel alloc] init];
    }
