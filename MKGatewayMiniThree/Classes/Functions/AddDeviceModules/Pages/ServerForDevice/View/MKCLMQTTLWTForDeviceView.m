...
@implementation MKCLMQTTLWTForDeviceViewModel
@end

@interface MKCLMQTTLWTForDeviceView ()
...
@implementation MKCLMQTTLWTForDeviceView
...
- (void)lwtButtonPressed {
    ...
    UIImage *lwtIcon = (self.lwtButton.selected ? LOADICON(@"MKGatewayMiniThree", @"MKCLMQTTLWTForDeviceView", @"cl_switchSelectedIcon.png") : LOADICON(@"MKGatewayMiniThree", @"MKCLMQTTLWTForDeviceView", @"cl_switchUnselectedIcon.png"));
    [self.lwtButton setImage:lwtIcon forState:UIControlStateNormal];
    if ([self.delegate respondsToSelector:@selector(cl_lwt_statusChanged:)]) {
        [self.delegate cl_lwt_statusChanged:self.lwtButton.selected];
    }
}

- (void)retainButtonPressed {
    self.retainButton.selected = !self.retainButton.selected;
    UIImage *retainIcon = (self.retainButton.selected ? LOADICON(@"MKGatewayMiniThree", @"MKCLMQTTLWTForDeviceView", @"cl_switchSelectedIcon.png") : LOADICON(@"MKGatewayMiniThree", @"MKCLMQTTLWTForDeviceView", @"cl_switchUnselectedIcon.png"));
    [self.retainButton setImage:retainIcon forState:UIControlStateNormal];
    if ([self.delegate respondsToSelector:@selector(cl_lwt_retainChanged:)]) {
        [self.delegate cl_lwt_retainChanged:self.retainButton.selected];
    }
}

- (void)segmentValueChanged {
    if ([self.delegate respondsToSelector:@selector(cl_lwt_qosChanged:)]) {
        [self.delegate cl_lwt_qosChanged:self.segment.selectedSegmentIndex];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCLMQTTLWTForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCLMQTTLWTForDeviceViewModel.class]) {
        return;
    }
...
- (UIButton *)lwtButton {
    if (!_lwtButton) {
        ...
        [_lwtButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLMQTTLWTForDeviceView", @"cl_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)retainButton {
    if (!_retainButton) {
        ...
        [_retainButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLMQTTLWTForDeviceView", @"cl_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (MKTextField *)topicTextField {
    if (!_topicTextField) {
        ...
        _topicTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(cl_lwt_topicChanged:)]) {
                [self.delegate cl_lwt_topicChanged:text];
            }
        };
...
- (MKTextField *)payloadTextField {
    if (!_payloadTextField) {
        ...
        _payloadTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(cl_lwt_payloadChanged:)]) {
                [self.delegate cl_lwt_payloadChanged:text];
            }
        };
