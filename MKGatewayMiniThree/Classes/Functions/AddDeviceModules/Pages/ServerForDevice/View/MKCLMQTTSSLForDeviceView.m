...
@implementation MKCLMQTTSSLForDeviceViewModel
@end

@interface MKCLMQTTSSLForDeviceView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKCLMQTTSSLForDeviceView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(cl_mqtt_sslParams_device_caFilePressed)]) {
            [self.delegate cl_mqtt_sslParams_device_caFilePressed];
        }
        return;
    }
    if (index == 1) {
        //Client Key File
        if ([self.delegate respondsToSelector:@selector(cl_mqtt_sslParams_device_clientKeyPressed)]) {
            [self.delegate cl_mqtt_sslParams_device_clientKeyPressed];
        }
        return;
    }
    //Client Cert File
    if ([self.delegate respondsToSelector:@selector(cl_mqtt_sslParams_device_clientCertPressed)]) {
        [self.delegate cl_mqtt_sslParams_device_clientCertPressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(cl_mqtt_sslParams_device_sslStatusChanged:)]) {
        [self.delegate cl_mqtt_sslParams_device_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(cl_mqtt_sslParams_device_certificateChanged:)]) {
            [self.delegate cl_mqtt_sslParams_device_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCLMQTTSSLForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCLMQTTSSLForDeviceViewModel.class]) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKGatewayMiniThree", @"MKCLMQTTSSLForDeviceView", @"cl_switchSelectedIcon.png") : LOADICON(@"MKGatewayMiniThree", @"MKCLMQTTSSLForDeviceView", @"cl_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLMQTTSSLForDeviceView", @"cl_switchUnselectedIcon.png") forState:UIControlStateNormal];
