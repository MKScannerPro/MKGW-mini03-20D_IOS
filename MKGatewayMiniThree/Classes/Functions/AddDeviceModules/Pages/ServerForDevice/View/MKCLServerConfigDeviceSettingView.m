//
//  MKCLServerConfigDeviceSettingView.m
...
@interface MKCLServerConfigDeviceSettingView ()
...
@implementation MKCLServerConfigDeviceSettingView
...
- (void)clearButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cl_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate cl_mqtt_deviecSetting_fileButtonPressed:2];
    }
}

- (void)exportButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cl_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate cl_mqtt_deviecSetting_fileButtonPressed:0];
    }
}

- (void)importButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cl_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate cl_mqtt_deviecSetting_fileButtonPressed:1];
    }
}
