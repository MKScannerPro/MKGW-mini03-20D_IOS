//
//  MKCLDeviceParamsListController.m
...
@interface MKCLDeviceParamsListController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation MKCLDeviceParamsListController

- (void)dealloc {
    NSLog(@"MKCLDeviceParamsListController销毁");
...
- (void)viewDidLoad {
    ...
    [MKCLDeviceMQTTParamsModel shared].deviceModel.deviceType = self.deviceType;
...
- (void)leftButtonMethod {
    ...
    [[MKCLCentralManager shared] disconnect];
    [MKCLDeviceMQTTParamsModel sharedDealloc];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //WIFI Settings
        MKCLBleWifiSettingsController *vc = [[MKCLBleWifiSettingsController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        //MQTT settings
        MKCLServerForDeviceController *vc = [[MKCLServerForDeviceController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 0) {
        //NTP & Timezone
        MKCLBleNTPTimezoneController *vc = [[MKCLBleNTPTimezoneController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 1) {
        //Scanner Filter
        MKCLBleScannerFilterController *vc = [[MKCLBleScannerFilterController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 2) {
        //Advertise iBeacon
        MKCLBleAdvBeaconController *vc = [[MKCLBleAdvBeaconController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 3) {
        //Device Information
        MKCLBleDeviceInfoController *vc = [[MKCLBleDeviceInfoController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)receiveDeviceOnline:(NSNotification *)note {
    ...
    if (!ValidDict(user) || !ValidStr(user[@"macAddress"]) || ![[MKCLDeviceMQTTParamsModel shared].deviceModel.macAddress isEqualToString:user[@"macAddress"]]) {
        return;
    }
...
- (void)connectButtonPressed {
    if (![MKCLDeviceMQTTParamsModel shared].wifiConfig || ![MKCLDeviceMQTTParamsModel shared].mqttConfig) {
        [self.view showCentralToast:@"Please configure WIFI and MQTT settings first!"];
        return;
    }
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKCLInterface cl_enterSTAModeWithSucBlock:^{
...
- (void)startMqttProcess {
    ...
    if (ValidStr([MKCLMQTTDataManager shared].serverParams.subscribeTopic)) {
        //查看是否设置了服务器的订阅topic
        topic = [MKCLMQTTDataManager shared].serverParams.subscribeTopic;
    }else {
        topic = [MKCLDeviceMQTTParamsModel shared].deviceModel.publishedTopic;
    }
    [self.progressView show];
    [[MKCLMQTTDataManager shared] subscriptions:@[topic]];
    [[NSNotificationCenter defaultCenter] addObserver:self
...
- (void)connectSuccess {
    ...
    MKCLDeviceModel *deviceModel = [[MKCLDeviceModel alloc] init];
    deviceModel.deviceType = [MKCLDeviceMQTTParamsModel shared].deviceModel.deviceType;
    deviceModel.clientID = [MKCLDeviceMQTTParamsModel shared].deviceModel.clientID;
    deviceModel.subscribedTopic = [MKCLDeviceMQTTParamsModel shared].deviceModel.subscribedTopic;
    deviceModel.publishedTopic = [MKCLDeviceMQTTParamsModel shared].deviceModel.publishedTopic;
    deviceModel.macAddress = [MKCLDeviceMQTTParamsModel shared].deviceModel.macAddress;
    deviceModel.deviceName = [MKCLDeviceMQTTParamsModel shared].deviceModel.deviceName;
    deviceModel.lwtStatus = [MKCLDeviceMQTTParamsModel shared].deviceModel.lwtStatus;
    deviceModel.lwtTopic = [MKCLDeviceMQTTParamsModel shared].deviceModel.lwtTopic;
        
    MKCLConnectSuccessController *vc = [[MKCLConnectSuccessController alloc] init];
