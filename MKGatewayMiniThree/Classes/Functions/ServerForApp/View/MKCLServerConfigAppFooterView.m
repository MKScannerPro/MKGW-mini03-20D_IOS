//
//  MKCLServerConfigAppFooterView.m
...
@implementation MKCLServerConfigAppFooterViewModel
@end
...
@interface MKCLServerConfigAppFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKCLMQTTSSLForAppViewDelegate>
...
@property (nonatomic, strong)MKCLMQTTSSLForAppView *sslView;
...
@implementation MKCLServerConfigAppFooterView
...
- (void)mk_mqtt_generalParams_cleanSessionStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(cl_mqtt_serverForApp_switchStatusChanged:statusID:)]) {
        [self.delegate cl_mqtt_serverForApp_switchStatusChanged:isOn statusID:0];
    }
}

- (void)mk_mqtt_generalParams_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(cl_mqtt_serverForApp_qosChanged:)]) {
        [self.delegate cl_mqtt_serverForApp_qosChanged:qos];
    }
}

- (void)mk_mqtt_generalParams_KeepAliveChanged:(NSString *)keepAlive {
    if ([self.delegate respondsToSelector:@selector(cl_mqtt_serverForApp_textFieldValueChanged:textID:)]) {
        [self.delegate cl_mqtt_serverForApp_textFieldValueChanged:keepAlive textID:0];
    }
}
...
- (void)mk_mqtt_userCredentials_userNameChanged:(NSString *)userName {
    if ([self.delegate respondsToSelector:@selector(cl_mqtt_serverForApp_textFieldValueChanged:textID:)]) {
        [self.delegate cl_mqtt_serverForApp_textFieldValueChanged:userName textID:1];
    }
}

- (void)mk_mqtt_userCredentials_passwordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(cl_mqtt_serverForApp_textFieldValueChanged:textID:)]) {
        [self.delegate cl_mqtt_serverForApp_textFieldValueChanged:password textID:2];
    }
}

#pragma mark - MKCLMQTTSSLForAppViewDelegate
- (void)cl_mqtt_sslParams_app_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(cl_mqtt_serverForApp_switchStatusChanged:statusID:)]) {
        [self.delegate cl_mqtt_serverForApp_switchStatusChanged:isOn statusID:1];
    }
}
...
- (void)cl_mqtt_sslParams_app_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(cl_mqtt_serverForApp_certificateChanged:)]) {
        [self.delegate cl_mqtt_serverForApp_certificateChanged:certificate];
    }
}

/// 用户点击选择了caFaile按钮
- (void)cl_mqtt_sslParams_app_caFilePressed {
    if ([self.delegate respondsToSelector:@selector(cl_mqtt_serverForApp_fileButtonPressed:)]) {
        [self.delegate cl_mqtt_serverForApp_fileButtonPressed:0];
    }
}
...
- (void)cl_mqtt_sslParams_app_clientFilePressed {
    if ([self.delegate respondsToSelector:@selector(cl_mqtt_serverForApp_fileButtonPressed:)]) {
        [self.delegate cl_mqtt_serverForApp_fileButtonPressed:1];
    }
}
...
- (void)clearButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cl_mqtt_serverForApp_bottomButtonPressed:)]) {
        [self.delegate cl_mqtt_serverForApp_bottomButtonPressed:2];
    }
}

- (void)exportButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cl_mqtt_serverForApp_bottomButtonPressed:)]) {
        [self.delegate cl_mqtt_serverForApp_bottomButtonPressed:0];
    }
}

- (void)importButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cl_mqtt_serverForApp_bottomButtonPressed:)]) {
        [self.delegate cl_mqtt_serverForApp_bottomButtonPressed:1];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKCLServerConfigAppFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCLServerConfigAppFooterViewModel.class]) {
        return;
    }
    ...
    MKCLMQTTSSLForAppViewModel *sslModel = [[MKCLMQTTSSLForAppViewModel alloc] init];
...
- (MKCLMQTTSSLForAppView *)sslView {
    if (!_sslView) {
        _sslView = [[MKCLMQTTSSLForAppView alloc] init];
