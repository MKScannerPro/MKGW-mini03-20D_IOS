...
@implementation MKCLMQTTSSLForAppViewModel
@end

@interface MKCLMQTTSSLForAppView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKCLMQTTSSLForAppView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(cl_mqtt_sslParams_app_caFilePressed)]) {
            [self.delegate cl_mqtt_sslParams_app_caFilePressed];
        }
        return;
    }
    //P12证书
    if ([self.delegate respondsToSelector:@selector(cl_mqtt_sslParams_app_clientFilePressed)]) {
        [self.delegate cl_mqtt_sslParams_app_clientFilePressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(cl_mqtt_sslParams_app_sslStatusChanged:)]) {
        [self.delegate cl_mqtt_sslParams_app_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(cl_mqtt_sslParams_app_certificateChanged:)]) {
            [self.delegate cl_mqtt_sslParams_app_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCLMQTTSSLForAppViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCLMQTTSSLForAppViewModel.class] || _dataModel.certificate < 0 || _dataModel.certificate > 2) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKGatewayMiniThree", @"MKCLMQTTSSLForAppView", @"cl_switchSelectedIcon.png") : LOADICON(@"MKGatewayMiniThree", @"MKCLMQTTSSLForAppView", @"cl_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLMQTTSSLForAppView", @"cl_switchUnselectedIcon.png") forState:UIControlStateNormal];
