...
@interface MKCLServerForAppController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKCLServerConfigAppFooterViewDelegate,
...
@property (nonatomic, strong)MKCLServerForAppModel *dataModel;

@property (nonatomic, strong)MKCLServerConfigAppFooterView *sslParamsView;

@property (nonatomic, strong)MKCLServerConfigAppFooterViewModel *sslParamsModel;
...
@implementation MKCLServerForAppController

- (void)dealloc {
    NSLog(@"MKCLServerForAppController销毁");
...
- (void)rightButtonMethod {
    ...
    [[MKCLMQTTDataManager shared] saveServerParams:self.dataModel];
    ...
    [[MKCLMQTTDataManager shared] connect];
}
...
#pragma mark - MKCLServerConfigAppFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl
- (void)cl_mqtt_serverForApp_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)cl_mqtt_serverForApp_qosChanged:(NSInteger)qos {
...
- (void)cl_mqtt_serverForApp_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keep alive
...
- (void)cl_mqtt_serverForApp_certificateChanged:(NSInteger)certificate {
...
- (void)cl_mqtt_serverForApp_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFaile
...
- (void)cl_mqtt_serverForApp_bottomButtonPressed:(NSInteger)index {
    if (index == 0) {
        //Export Demo File
...
- (void)cl_selectedServerParams:(NSString *)fileName {
    [MKCLExcelDataManager parseAppExcel:fileName
                                sucBlock:^(NSDictionary * _Nonnull returnData) {
        MKCLServerForAppModel *model = [MKCLServerForAppModel mk_modelWithJSON:returnData];
...
- (void)serverManagerStateChanged {
    ...
    if ([MKCLMQTTDataManager shared].state == MKCLMQTTSessionManagerStateConnected) {
        ...
        [[NSNotificationCenter defaultCenter] removeObserver:self
                                                        name:MKCLMQTTSessionManagerStateChangedNotification
                                                      object:nil];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_cl_needReloadTopicsNotification"
                                                            object:nil];
...
//    }
    ...
    [MKCLExcelDataManager exportAppExcel:self.dataModel
                                 sucBlock:^{
        [[MKHudManager share] hide];
        [self sharedExcel];
    }
...
- (void)connectFailed {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self
                                                    name:MKCLMQTTSessionManagerStateChangedNotification
                                                  object:nil];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"MKCLMQTTServerConnectFailedNotification" object:nil];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLServerForAppController", @"cl_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCLServerForAppModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCLServerForAppModel alloc] init];
    }
    return _dataModel;
}
...
- (MKCLServerConfigAppFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKCLServerConfigAppFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 230.f)];
...
- (MKCLServerConfigAppFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKCLServerConfigAppFooterViewModel alloc] init];
    }
