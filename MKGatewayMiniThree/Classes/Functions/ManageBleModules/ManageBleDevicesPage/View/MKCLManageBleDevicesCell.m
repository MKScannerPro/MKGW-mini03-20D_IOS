...
@implementation MKCLManageBleDevicesCellModel
@end

@interface MKCLManageBleDevicesCell ()
...
@implementation MKCLManageBleDevicesCell

+ (MKCLManageBleDevicesCell *)initCellWithTableView:(UITableView *)tableView {
    MKCLManageBleDevicesCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCLManageBleDevicesCellIdenty"];
    if (!cell) {
        cell = [[MKCLManageBleDevicesCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCLManageBleDevicesCellIdenty"];
    }
    return cell;
}
...
- (void)connectButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cl_manageBleDevicesCellConnectButtonPressed:typeCode:)]) {
        [self.delegate cl_manageBleDevicesCellConnectButtonPressed:SafeStr(self.dataModel.macAddress)
                                                          typeCode:self.dataModel.typeCode];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCLManageBleDevicesCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCLManageBleDevicesCellModel.class]) {
        return;
    }
