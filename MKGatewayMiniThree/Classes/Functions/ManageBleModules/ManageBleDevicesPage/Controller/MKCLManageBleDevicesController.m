//
//  MKCLManageBleDevicesController.m
...
@interface MKCLManageBleDevicesController ()<UITableViewDelegate,
UITableViewDataSource,
MKCLManageBleDevicesSearchButtonDelegate,
MKCLManageBleDevicesCellDelegate>

@property (nonatomic, strong)MKBaseTableView *tableView;

@property (nonatomic, strong)MKCLManageBleDevicesSearchButtonModel *buttonModel;

@property (nonatomic, strong)MKCLManageBleDevicesSearchButton *searchButton;
...
@implementation MKCLManageBleDevicesController

- (void)dealloc {
    NSLog(@"MKCLManageBleDevicesController销毁");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKCLManageBleDevicesCell *cell = [MKCLManageBleDevicesCell initCellWithTableView:tableView];
    cell.dataModel = self.dataList[indexPath.row];
    cell.delegate = self;
    return cell;
}

#pragma mark - MKCLManageBleDevicesSearchButtonDelegate
- (void)cl_scanSearchButtonMethod {
    [MKCLManageBleDeviceSearchView showSearchName:self.buttonModel.searchName
...
#pragma mark - MKCLManageBleDevicesCellDelegate
/// 用户点击了链接按钮
...
- (void)cl_manageBleDevicesCellConnectButtonPressed:(NSString *)macAddress typeCode:(NSInteger)typeCode; {
    if (typeCode == 7) {
        ...
        [alertView showAlertWithTitle:@"Enter password" message:@"" notificationName:@"mk_cl_needDismissAlert"];
        return;
    }
    //Normal
...
- (void)addNotes {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDeviceDatas:)
                                                 name:MKCLReceiveDeviceDatasNotification
                                               object:nil];
}
...
- (void)updateCellModelWithMac:(NSString *)macAddress
    ...
    MKCLManageBleDevicesCellModel *cellModel = [[MKCLManageBleDevicesCellModel alloc] init];
...
- (void)connectBXPButtonWithPassword:(NSString *)password bleMac:(NSString *)bleMac {
    ...
    [MKCLMQTTInterface cl_connectBXPButtonWithPassword:password bleMac:bleMac macAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)connectNormalDeviceWithBleMac:(NSString *)macAddress {
    [[MKHudManager share] showHUDWithTitle:@"Connecting..." inView:self.view isPenetration:NO];
    [MKCLMQTTInterface cl_connectNormalBleDeviceWithBleMac:macAddress macAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            //连接失败
            [self.view showCentralToast:returnData[@"data"][@"result_msg"]];
...
- (MKCLManageBleDevicesSearchButton *)searchButton {
    if (!_searchButton) {
        _searchButton = [[MKCLManageBleDevicesSearchButton alloc] init];
        _searchButton.delegate = self;
    }
    return _searchButton;
}

- (MKCLManageBleDevicesSearchButtonModel *)buttonModel {
    if (!_buttonModel) {
        _buttonModel = [[MKCLManageBleDevicesSearchButtonModel alloc] init];
