...
@implementation MKCLButtonFirmwareCellModel
@end

@interface MKCLButtonFirmwareCell ()
...
@implementation MKCLButtonFirmwareCell

+ (MKCLButtonFirmwareCell *)initCellWithTableView:(UITableView *)tableView {
    MKCLButtonFirmwareCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCLButtonFirmwareCellIdenty"];
    if (!cell) {
        cell = [[MKCLButtonFirmwareCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCLButtonFirmwareCellIdenty"];
    }
    return cell;
}
...
- (void)rightButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cl_buttonFirmwareCell_buttonAction:)]) {
        [self.delegate cl_buttonFirmwareCell_buttonAction:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCLButtonFirmwareCellModel *)dataModel {
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCLButtonFirmwareCellModel.class]) {
        return;
    }
