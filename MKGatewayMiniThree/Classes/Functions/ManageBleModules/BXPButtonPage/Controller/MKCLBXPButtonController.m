...
@interface MKCLBXPButtonController ()<UITableViewDelegate,
UITableViewDataSource,
MKButtonMsgCellDelegate,
MKCLButtonFirmwareCellDelegate>
...
@implementation MKCLBXPButtonController

- (void)dealloc {
    NSLog(@"MKCLBXPButtonController销毁");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKCLButtonFirmwareCell *cell = [MKCLButtonFirmwareCell initCellWithTableView:tableView];
...
#pragma mark - MKCLButtonFirmwareCellDelegate
- (void)cl_buttonFirmwareCell_buttonAction:(NSInteger)index {
    if (index == 0) {
        //DFU
        MKCLButtonDFUController *vc = [[MKCLButtonDFUController alloc] init];
...
- (void)readDatasFromDevice {
    ...
    [MKCLMQTTInterface cl_readBXPButtonConnectedStatusWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)dismissAlarmStatus {
    ...
    [MKCLMQTTInterface cl_dismissBXPButtonAlarmStatusWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            [self.view showCentralToast:@"Read Failed"];
            return;
...
- (void)sendLedReminder:(NSString *)interval duration:(NSString *)duration color:(NSInteger)color {
    ...
    [MKCLMQTTInterface cl_configDeviceLedReminderWithBleMac:self.deviceBleInfo[@"data"][@"mac"] interval:[interval integerValue] duration:[duration integerValue] macAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
...
- (void)sendBuzzerReminder:(NSString *)interval duration:(NSString *)duration {
    ...
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKCLMQTTInterface cl_configDeviceBuzzerReminderWithBleMac:self.deviceBleInfo[@"data"][@"mac"] interval:[interval integerValue] duration:[duration integerValue] macAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            [self.view showCentralToast:@"setup failed!"];
            return;
...
- (void)addNotes {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDisconnect:)
                                                 name:MKCLReceiveGatewayDisconnectBXPButtonNotification
                                               object:nil];
}

- (void)showLedReminderAlert {
    MKCLReminderAlertViewModel *dataModel = [[MKCLReminderAlertViewModel alloc] init];
    dataModel.title = @"LED Reminder";
    dataModel.intervalMsg = @"Blinking interval";
    dataModel.durationMsg = @"Blinking duration";
    MKCLReminderAlertView *alertView = [[MKCLReminderAlertView alloc] init];
...
- (void)loadSection1Datas {
    MKCLButtonFirmwareCellModel *cellModel = [[MKCLButtonFirmwareCellModel alloc] init];
    cellModel.index = 0;
