...
@interface MKCLButtonDFUController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCLButtonDFUModel *dataModel;
...
@implementation MKCLButtonDFUController

- (void)dealloc {
    NSLog(@"MKCLButtonDFUController销毁");
...
- (void)addNotes {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDfuProgress:)
                                                 name:MKCLReceiveBxpButtonDfuProgressNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDfuResult:)
                                                 name:MKCLReceiveBxpButtonDfuResultNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
...
- (void)gotoLastPage {
    ...
    for (UIViewController *vc in self.navigationController.viewControllers) {
        if ([vc isKindOfClass:NSClassFromString(@"MKCLManageBleDevicesController")]) {
            //如果是从Manage BLE devices页面进来的
            [self popToViewControllerWithClassName:@"MKCLManageBleDevicesController"];
            break;
        }
        if ([vc isKindOfClass:NSClassFromString(@"MKCLDeviceDataController")]) {
            //如果是从扫描页面进来的
...
- (MKCLButtonDFUModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCLButtonDFUModel alloc] init];
