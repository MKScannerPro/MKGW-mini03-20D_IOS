...
@implementation MKCLNormalConnectedCellModel
...
@interface MKCLNormalConnectedCell ()
...
@implementation MKCLNormalConnectedCell

+ (MKCLNormalConnectedCell *)initCellWithTableView:(UITableView *)tableView {
    MKCLNormalConnectedCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCLNormalConnectedCellIdenty"];
    if (!cell) {
        cell = [[MKCLNormalConnectedCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCLNormalConnectedCellIdenty"];
    }
    return cell;
}
...
- (void)writeButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cl_normalConnectedCell_writeButtonPressed:row:serverUUID:characteristic:)]) {
        [self.delegate cl_normalConnectedCell_writeButtonPressed:self.dataModel.section
...
- (void)readButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cl_normalConnectedCell_readButtonPressed:row:serverUUID:characteristic:)]) {
        [self.delegate cl_normalConnectedCell_readButtonPressed:self.dataModel.section
                                                            row:self.dataModel.row
                                                     serverUUID:self.dataModel.server
                                                 characteristic:self.dataModel.characteristic];
    }
...
- (void)notifyButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cl_normalConnectedCell_notifyButtonPressed:section:row:serverUUID:characteristic:)]) {
        [self.delegate cl_normalConnectedCell_notifyButtonPressed:!self.notifyButton.selected
...
#pragma mark - setter
- (void)setDataModel:(MKCLNormalConnectedCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCLNormalConnectedCellModel.class]) {
        return;
    }
    ...
    [self.notifyButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLNormalConnectedCell", @"cl_characteristic_notifyIcon.png") forState:UIControlStateNormal];
    self.notifyButton.selected = NO;
    if (_dataModel.notifyStatus == 1) {
        //打开了监听
        [self.notifyButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLNormalConnectedCell", @"cl_characteristic_receiveIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)writeButton {
    if (!_writeButton) {
        ...
        [_writeButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLNormalConnectedCell", @"cl_characteristic_writeIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)readButton {
    if (!_readButton) {
        ...
        [_readButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLNormalConnectedCell", @"cl_characteristic_readIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)notifyButton {
    if (!_notifyButton) {
        ...
        [_notifyButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLNormalConnectedCell", @"cl_characteristic_notifyIcon.png") forState:UIControlStateNormal];
