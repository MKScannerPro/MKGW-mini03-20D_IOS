...
@interface MKCLFilterByTagModel ()
...
@implementation MKCLFilterByTagModel
...
- (BOOL)readFilterByTag {
    ...
    [MKCLMQTTInterface cl_readFilterBXPTagWithMacAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByTag:(NSArray <NSString *>*)tagList {
    __block BOOL success = NO;
    [MKCLMQTTInterface cl_configFilterByTag:self.isOn preciseMatch:self.precise reverseFilter:self.reverse tagIDList:tagList macAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
