...
@interface MKCLFilterByTagController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCLFilterByTagModel *dataModel;

@end

@implementation MKCLFilterByTagController

- (void)dealloc {
    NSLog(@"MKCLFilterByTagController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLFilterByTagController", @"cl_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCLFilterByTagModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCLFilterByTagModel alloc] init];
    }
    return _dataModel;
}
