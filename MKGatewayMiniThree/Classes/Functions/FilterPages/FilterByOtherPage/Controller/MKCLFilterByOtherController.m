...
@interface MKCLFilterByOtherController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCLFilterByOtherModel *dataModel;

@end

@implementation MKCLFilterByOtherController

- (void)dealloc {
    NSLog(@"MKCLFilterByOtherController销毁");
}
...
- (void)configDataToDevice {
    ...
    for (NSInteger i = 0; i < self.section1List.count; i ++) {
        ...
        MKCLFilterRawAdvDataModel *model = [[MKCLFilterRawAdvDataModel alloc] init];
...
- (mk_cl_filterByOther)loadCurrentRelationship {
    MKTextButtonCellModel *cellModel = self.section2List[0];
    if (self.section1List.count == 1) {
        //@[@"A"];
        return mk_cl_filterByOther_A;
    }
    if (self.section1List.count == 2) {
        //@[@"A & B",@"A | B"]
        if (cellModel.dataListIndex == 0) {
            return mk_cl_filterByOther_AB;
        }
        return mk_cl_filterByOther_AOrB;
    }
    if (self.section1List.count == 3) {
        //@[@"A & B & C",@"(A & B) | C",@"A | B | C"]
        if (cellModel.dataListIndex == 0) {
            return mk_cl_filterByOther_ABC;
        }
        if (cellModel.dataListIndex == 1) {
            return mk_cl_filterByOther_ABOrC;
        }
        if (cellModel.dataListIndex == 2) {
            return mk_cl_filterByOther_AOrBOrC;
        }
    }
    return mk_cl_filterByOther_AOrBOrC;
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLFilterByOtherController", @"cl_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCLFilterByOtherModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCLFilterByOtherModel alloc] init];
    }
    return _dataModel;
}
