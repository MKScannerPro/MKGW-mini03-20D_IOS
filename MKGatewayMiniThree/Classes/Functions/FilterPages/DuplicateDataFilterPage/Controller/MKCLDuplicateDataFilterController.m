//
//  MKCLDuplicateDataFilterController.m
...
@interface MKCLDuplicateDataFilterController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCLDuplicateDataFilterModel *dataModel;

@end

@implementation MKCLDuplicateDataFilterController

- (void)dealloc {
    NSLog(@"MKCLDuplicateDataFilterController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLDuplicateDataFilterController", @"cl_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCLDuplicateDataFilterModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCLDuplicateDataFilterModel alloc] init];
    }
    return _dataModel;
}
