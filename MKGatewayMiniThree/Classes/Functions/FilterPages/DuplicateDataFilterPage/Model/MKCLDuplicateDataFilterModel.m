//
//  MKCLDuplicateDataFilterModel.m
...
@interface MKCLDuplicateDataFilterModel ()
...
@implementation MKCLDuplicateDataFilterModel
...
- (BOOL)readDuplicateDataFilter {
    ...
    [MKCLMQTTInterface cl_readDuplicateDataFilterDatasWithMacAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configDuplicateDataFilter {
    __block BOOL success = NO;
    [MKCLMQTTInterface cl_configDuplicateDataFilter:self.rule period:[self.time longLongValue] macAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
