...
@interface MKCLFilterByUIDModel ()
...
@implementation MKCLFilterByUIDModel
...
- (BOOL)readFilterByUID {
    ...
    [MKCLMQTTInterface cl_readFilterByUIDWithMacAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByUID {
    __block BOOL success = NO;
    [MKCLMQTTInterface cl_configFilterByUID:self.isOn namespaceID:self.namespaceID instanceID:self.instanceID macAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
