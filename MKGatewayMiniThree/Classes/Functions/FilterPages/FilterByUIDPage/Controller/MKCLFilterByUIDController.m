...
@interface MKCLFilterByUIDController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCLFilterByUIDModel *dataModel;

@end

@implementation MKCLFilterByUIDController

- (void)dealloc {
    NSLog(@"MKCLFilterByUIDController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLFilterByUIDController", @"cl_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCLFilterByUIDModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCLFilterByUIDModel alloc] init];
    }
    return _dataModel;
}
