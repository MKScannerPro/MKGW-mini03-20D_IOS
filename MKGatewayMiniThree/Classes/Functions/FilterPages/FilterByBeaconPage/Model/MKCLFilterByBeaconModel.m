...
@interface MKCLFilterByBeaconModel ()

@property (nonatomic, strong)dispatch_queue_t readQueue;

@property (nonatomic, strong)dispatch_semaphore_t semaphore;

@property (nonatomic, assign)mk_cl_filterByBeaconPageType pageType;

@end

@implementation MKCLFilterByBeaconModel
...
- (void)readDataWithSucBlock:(void (^)(void))sucBlock failedBlock:(void (^)(NSError *error))failedBlock {
    dispatch_async(self.readQueue, ^{
        if (self.pageType == mk_cl_filterByBeaconPageType_beacon) {
        ...
        /*
        else if (self.pageType == mk_cl_filterByBeaconPageType_MKBeacon) {
            if (![self readFilterByMKBeacon]) {
                [self operationFailedBlockWithMsg:@"Read Filter iBeacon Error" block:failedBlock];
                return;
            }
        }else if (self.pageType == mk_cl_filterByBeaconPageType_MKBeaconAcc) {
...
- (BOOL)readFilterByBeacon {
    ...
    [MKCLMQTTInterface cl_readFilterByBeaconWithMacAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByBeacon {
    ...
    [MKCLMQTTInterface cl_configFilterByBeacon:self.isOn minMinor:tempMinMinor maxMinor:tempMaxMinor minMajor:tempMinMajor maxMajor:tempMaxMajor uuid:self.uuid macAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readFilterByMKBeacon {
    __block BOOL success = NO;
    [MKCLMQTTInterface cl_readFilterByMKBeaconWithDeviceID:self.deviceID macAddress:self.macAddress topic:self.topic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)readFilterByMKBeaconACC {
    __block BOOL success = NO;
    [MKCLMQTTInterface cl_readFilterByMKBeaconACCWithDeviceID:self.deviceID macAddress:self.macAddress topic:self.topic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        self.isOn = ([returnData[@"data"][@"switch"] integerValue] == 1);
        self.minMinor = [NSString stringWithFormat:@"%@",returnData[@"data"][@"min_minor"]];
        self.maxMinor = [NSString stringWithFormat:@"%@",returnData[@"data"][@"max_minor"]];
