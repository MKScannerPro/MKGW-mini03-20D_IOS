//
//  MKCLFilterByRawDataController.m
...
@property (nonatomic, strong)MKCLFilterByRawDataModel *dataModel;

@end

@implementation MKCLFilterByRawDataController

- (void)dealloc {
    NSLog(@"MKCLFilterByRawDataController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //iBeacon
        MKCLFilterByBeaconController *vc = [[MKCLFilterByBeaconController alloc] init];
        vc.pageType = mk_cl_filterByBeaconPageType_beacon;
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        //Eddystone-UID
        MKCLFilterByUIDController *vc = [[MKCLFilterByUIDController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 2) {
        //Eddystone-URL
        MKCLFilterByURLController *vc = [[MKCLFilterByURLController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 3) {
        //Eddystone-TLM
        MKCLFilterByTLMController *vc = [[MKCLFilterByTLMController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 0) {
        //BXP-Button
        MKCLFilterByButtonController *vc = [[MKCLFilterByButtonController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 1) {
        //BXP-Tag
        MKCLFilterByTagController *vc = [[MKCLFilterByTagController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 2) {
        //PIR
        MKCLFilterByPirController *vc = [[MKCLFilterByPirController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
...
- (void)configFilterBXPDeviceInfo:(BOOL)isOn {
    ...
    [MKCLMQTTInterface cl_configFilterFilterBXPDeviceInfo:isOn macAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)configFilterBXPACC:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKCLMQTTInterface cl_configFilterBXPAcc:isOn macAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)configFilterBXPTH:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKCLMQTTInterface cl_configFilterBXPTH:isOn macAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (MKCLFilterByRawDataModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCLFilterByRawDataModel alloc] init];
    }
    return _dataModel;
}
