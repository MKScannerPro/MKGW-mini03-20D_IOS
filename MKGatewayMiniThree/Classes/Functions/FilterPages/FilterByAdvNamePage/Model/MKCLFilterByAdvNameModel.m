...
@interface MKCLFilterByAdvNameModel ()
...
@implementation MKCLFilterByAdvNameModel
...
- (BOOL)readFilterByName {
    ...
    [MKCLMQTTInterface cl_readFilterByADVNameWithMacAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByName:(NSArray <NSString *>*)nameList {
    __block BOOL success = NO;
    [MKCLMQTTInterface cl_configFilterByADVName:nameList preciseMatch:self.preciseMatch reverseFilter:self.reverseFilter macAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
