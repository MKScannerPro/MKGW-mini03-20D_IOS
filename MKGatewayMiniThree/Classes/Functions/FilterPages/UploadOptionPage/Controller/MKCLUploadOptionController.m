...
@interface MKCLUploadOptionController ()<UITableViewDelegate,
UITableViewDataSource,
MKNormalSliderCellDelegate,
MKCLFilterCellDelegate>
...
@property (nonatomic, strong)MKCLUploadOptionModel *dataModel;

@end

@implementation MKCLUploadOptionController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKCLFilterCell *cell = [MKCLFilterCell initCellWithTableView:tableView];
...
- (void)cl_filterValueChanged:(NSInteger)dataListIndex index:(NSInteger)index {
    ...
    if (index == 1) {
        ...
        MKCLFilterCellModel *cellModel = self.section3List[0];
        cellModel.dataListIndex = dataListIndex;
        return;
    }
}
...
- (void)filterByMACAddress {
    MKCLFilterByMacController *vc = [[MKCLFilterByMacController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)filterByADVName {
    MKCLFilterByAdvNameController *vc = [[MKCLFilterByAdvNameController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)filterByRawData {
    MKCLFilterByRawDataController *vc = [[MKCLFilterByRawDataController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)duplicateDataFilter {
    MKCLDuplicateDataFilterController *vc = [[MKCLDuplicateDataFilterController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)uploadDataOption {
    MKCLUploadDataOptionController *vc = [[MKCLUploadDataOptionController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)loadSection1Datas {
    MKCLFilterCellModel *cellModel = [[MKCLFilterCellModel alloc] init];
    cellModel.index = 0;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLUploadOptionController", @"cl_saveIcon.png")
...
- (MKCLUploadOptionModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCLUploadOptionModel alloc] init];
    }
    return _dataModel;
}
