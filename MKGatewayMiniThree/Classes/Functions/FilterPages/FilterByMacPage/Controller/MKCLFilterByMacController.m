...
@interface MKCLFilterByMacController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCLFilterByMacModel *dataModel;

@end

@implementation MKCLFilterByMacController
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLFilterByMacController", @"cl_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCLFilterByMacModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCLFilterByMacModel alloc] init];
    }
    return _dataModel;
}
