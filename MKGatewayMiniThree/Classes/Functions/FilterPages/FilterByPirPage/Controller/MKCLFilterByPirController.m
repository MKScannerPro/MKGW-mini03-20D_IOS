...
@interface MKCLFilterByPirController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCLFilterByPirModel *dataModel;

@end

@implementation MKCLFilterByPirController

- (void)dealloc {
    NSLog(@"MKCLFilterByPirController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLFilterByPirController", @"cl_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCLFilterByPirModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCLFilterByPirModel alloc] init];
    }
    return _dataModel;
}
