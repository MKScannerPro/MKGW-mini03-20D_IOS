...
@interface MKCLFilterByTofModel ()
...
@implementation MKCLFilterByTofModel
...
- (BOOL)readFilterByTof {
    ...
    [MKCLMQTTInterface cl_readFilterByTofWithMacAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByTof:(NSArray <NSString *>*)codeList {
    __block BOOL success = NO;
    [MKCLMQTTInterface cl_configFilterByTofList:codeList isOn:self.isOn macAddress:[MKCLDeviceModeManager shared].macAddress topic:[MKCLDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
