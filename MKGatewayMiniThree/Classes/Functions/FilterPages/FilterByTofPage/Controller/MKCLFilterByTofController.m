...
@interface MKCLFilterByTofController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCLFilterByTofModel *dataModel;

@end

@implementation MKCLFilterByTofController

- (void)dealloc {
    NSLog(@"MKCLFilterByTofController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniThree", @"MKCLFilterByTofController", @"cl_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCLFilterByTofModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCLFilterByTofModel alloc] init];
    }
    return _dataModel;
}
