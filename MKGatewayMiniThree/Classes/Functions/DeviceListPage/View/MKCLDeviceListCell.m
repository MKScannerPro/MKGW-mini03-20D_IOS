...
@implementation MKCLDeviceListCell

+ (MKCLDeviceListCell *)initCellWithTableView:(UITableView *)tableView {
    MKCLDeviceListCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCLDeviceListCellIdenty"];
    if (!cell) {
        cell = [[MKCLDeviceListCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCLDeviceListCellIdenty"];
    }
    return cell;
}
...
- (void)longPressEventAction:(UILongPressGestureRecognizer *)longPress {
    if (longPress.state == UIGestureRecognizerStateBegan && [self.delegate respondsToSelector:@selector(cl_cellDeleteButtonPressed:)]) {
        [self.delegate cl_cellDeleteButtonPressed:self.indexPath.row];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCLDeviceListModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCLDeviceModel.class]) {
        return;
    }
    self.deviceNameLabel.text = SafeStr(_dataModel.deviceName);
    self.stateLabel.text = (_dataModel.onLineState == MKCLDeviceModelStateOnline ? @"Online" : @"Offline");
    self.stateLabel.textColor = (_dataModel.onLineState == MKCLDeviceModelStateOnline ? NAVBAR_COLOR_MACROS : UIColorFromRGB(0xcccccc));
    self.macLabel.text = SafeStr(_dataModel.macAddress);
    if (dataModel.onLineState == MKCLDeviceModelStateOffline) {
        //设备离线
        self.wifiIcon.image = LOADICON(@"MKGatewayMiniThree", @"MKCLDeviceListCell", @"cl_offline_wifi.png");
        return;
    }
    //设备在线
    if (_dataModel.wifiLevel >= -50) {
        //Good
        self.wifiIcon.image = LOADICON(@"MKGatewayMiniThree", @"MKCLDeviceListCell", @"cl_good_wifi.png");
        return;
    }
    if (_dataModel.wifiLevel >= -65 && _dataModel.wifiLevel < -50) {
        //Medium
        self.wifiIcon.image = LOADICON(@"MKGatewayMiniThree", @"MKCLDeviceListCell", @"cl_medium_wifi.png");
        return;
    }
    if (_dataModel.wifiLevel < -65) {
        //Poor
        self.wifiIcon.image = LOADICON(@"MKGatewayMiniThree", @"MKCLDeviceListCell", @"cl_poor_wifi.png");
        return;
    }
}
...
- (UIImageView *)wifiIcon {
    if (!_wifiIcon) {
        ...
        _wifiIcon.image = LOADICON(@"MKGatewayMiniThree", @"MKCLDeviceListCell", @"cl_offline_wifi.png");
    }
    return _wifiIcon;
}
...
- (UIImageView *)rightIcon {
    if (!_rightIcon) {
        ...
        _rightIcon.image = LOADICON(@"MKGatewayMiniThree", @"MKCLDeviceListCell", @"cl_goNextButton.png");
    }
    return _rightIcon;
}
