//
//  MKCLDeviceDataPageHeaderView.m
...
@implementation MKCLDeviceDataPageHeaderViewModel
@end

@interface MKCLDeviceDataPageManageBleButton : UIControl
...
@implementation MKCLDeviceDataPageManageBleButton
...
- (UIImageView *)rightIcon {
    if (!_rightIcon) {
        ...
        _rightIcon.image = LOADICON(@"MKGatewayMiniThree", @"MKCLDeviceDataPageManageBleButton", @"cl_goNextButton.png");
    }
    return _rightIcon;
}
...
@interface MKCLDeviceDataPageHeaderView ()
...
@property (nonatomic, strong)MKCLDeviceDataPageManageBleButton *manageBleButton;

@end

@implementation MKCLDeviceDataPageHeaderView
...
- (void)uploadButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cl_updateLoadButtonAction)]) {
        [self.delegate cl_updateLoadButtonAction];
    }
}

- (void)scannerButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cl_scannerStatusChanged:)]) {
        [self.delegate cl_scannerStatusChanged:!self.scannerButton.selected];
    }
}

- (void)manageBleButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cl_manageBleDeviceAction)]) {
        [self.delegate cl_manageBleDeviceAction];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCLDeviceDataPageHeaderViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCLDeviceDataPageHeaderViewModel.class]) {
        return;
    }
    self.scannerButton.selected = _dataModel.isOn;
    UIImage *image = (self.scannerButton.selected ? LOADICON(@"MKGatewayMiniThree", @"MKCLDeviceDataPageHeaderView", @"cl_switchSelectedIcon.png") : LOADICON(@"MKGatewayMiniThree", @"MKCLDeviceDataPageHeaderView", @"cl_switchUnselectedIcon.png"));
...
- (MKCLDeviceDataPageManageBleButton *)manageBleButton {
    if (!_manageBleButton) {
        _manageBleButton = [[MKCLDeviceDataPageManageBleButton alloc] init];
