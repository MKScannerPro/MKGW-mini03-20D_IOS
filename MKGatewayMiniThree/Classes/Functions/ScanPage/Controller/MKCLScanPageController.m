...
static NSString *const localPasswordKey = @"mk_cl_passwordKey";

static NSTimeInterval const kRefreshInterval = 0.5f;

@interface MKCLScanPageController ()<UITableViewDelegate,
UITableViewDataSource,
mk_cl_centralManagerScanDelegate>
...
@implementation MKCLScanPageController

- (void)dealloc {
    NSLog(@"MKCLScanPageController销毁");
    //移除runloop的监听
    CFRunLoopRemoveObserver(CFRunLoopGetCurrent(), self.observerRef, kCFRunLoopCommonModes);
    [[MKCLCentralManager shared] stopScan];
    [MKCLCentralManager sharedDealloc];
}
...
- (void)viewDidLoad {
    ...
    [MKCLCentralManager shared].delegate = self;
}

#pragma mark - super method
- (void)rightButtonMethod {
    if ([MKCLCentralManager shared].centralStatus != mk_cl_centralManagerStatusEnable) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    MKCLScanPageCell *cell = [MKCLScanPageCell initCellWithTableView:tableView];
    cell.dataModel = self.dataList[indexPath.row];
    return cell;
}

#pragma mark - mk_cl_centralManagerScanDelegate

- (void)mk_cl_receiveDevice:(NSDictionary *)deviceModel {
    MKCLScanPageModel *dataModel = [MKCLScanPageModel mk_modelWithJSON:deviceModel];
...
- (void)scanTimerRun{
    ...
    [[MKCLCentralManager shared] startScan];
...
- (void)updateDataWithDeviceModel:(MKCLScanPageModel *)deviceModel{
...
#pragma mark - 连接部分
- (void)connectDeviceWithModel:(MKCLScanPageModel *)deviceModel {
    //停止扫描
    [self.refreshIcon.layer removeAnimationForKey:@"mk_refreshAnimationKey"];
    [[MKCLCentralManager shared] stopScan];
    if (self.scanTimer) {
...
- (void)connectDevice:(MKCLScanPageModel *)deviecModel {
    ...
    [[MKCLCentralManager shared] connectPeripheral:deviecModel.peripheral password:password sucBlock:^(CBPeripheral * _Nonnull peripheral) {
...
- (void)pushMQTTForDevicePage:(NSString *)deviceType {
    MKCLDeviceParamsListController *vc = [[MKCLDeviceParamsListController alloc] init];
...
- (UIImageView *)refreshIcon {
    if (!_refreshIcon) {
        ...
        _refreshIcon.image = LOADICON(@"MKGatewayMiniThree", @"MKCLScanPageController", @"cl_scanRefresh.png");
    }
    return _refreshIcon;
}
