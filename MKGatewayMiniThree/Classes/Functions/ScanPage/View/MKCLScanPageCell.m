...
@implementation MKCLScanPageCell

+ (MKCLScanPageCell *)initCellWithTableView:(UITableView *)tableView {
    MKCLScanPageCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCLScanPageCellIdenty"];
    if (!cell) {
        cell = [[MKCLScanPageCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCLScanPageCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKCLScanPageModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCLScanPageModel.class]) {
        return;
    }
