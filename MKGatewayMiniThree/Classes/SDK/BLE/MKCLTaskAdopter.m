...
NSString *const mk_cl_totalNumKey = @"mk_cl_totalNumKey";
NSString *const mk_cl_totalIndexKey = @"mk_cl_totalIndexKey";
NSString *const mk_cl_contentKey = @"mk_cl_contentKey";

@implementation MKCLTaskAdopter
...
+ (NSDictionary *)parseReadDataWithCharacteristic:(CBCharacteristic *)characteristic {
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"2A24"]]) {
        ...
        return [self dataParserGetDataSuccess:@{@"modeID":tempString} operationID:mk_cl_taskReadDeviceModelOperation];
    }
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"2A26"]]) {
        //firmware
        NSString *tempString = [[NSString alloc] initWithData:readData encoding:NSUTF8StringEncoding];
        return [self dataParserGetDataSuccess:@{@"firmware":tempString} operationID:mk_cl_taskReadFirmwareOperation];
    }
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"2A27"]]) {
        //hardware
        NSString *tempString = [[NSString alloc] initWithData:readData encoding:NSUTF8StringEncoding];
        return [self dataParserGetDataSuccess:@{@"hardware":tempString} operationID:mk_cl_taskReadHardwareOperation];
    }
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"2A28"]]) {
        //soft ware
        NSString *tempString = [[NSString alloc] initWithData:readData encoding:NSUTF8StringEncoding];
        return [self dataParserGetDataSuccess:@{@"software":tempString} operationID:mk_cl_taskReadSoftwareOperation];
    }
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"2A29"]]) {
        //manufacturerKey
        NSString *tempString = [[NSString alloc] initWithData:readData encoding:NSUTF8StringEncoding];
        return [self dataParserGetDataSuccess:@{@"manufacturer":tempString} operationID:mk_cl_taskReadManufacturerOperation];
    }
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"AA00"]]) {
        ...
        return [self dataParserGetDataSuccess:@{@"state":state} operationID:mk_cl_connectPasswordOperation];
    }
...
+ (NSDictionary *)parseCustomReadData:(NSString *)content cmd:(NSString *)cmd data:(NSData *)data {
    mk_cl_taskOperationID operationID = mk_cl_defaultTaskOperationID;
    ...
    if ([cmd isEqualToString:@"02"]) {
        ...
        operationID = mk_cl_taskReadDeviceNameOperation;
        ...
        operationID = mk_cl_taskReadDeviceMacAddressOperation;
    }else if ([cmd isEqualToString:@"0a"]) {
        //读取WIFI STA MAC
        NSString *macAddress = [NSString stringWithFormat:@"%@:%@:%@:%@:%@:%@",[content substringWithRange:NSMakeRange(0, 2)],[content substringWithRange:NSMakeRange(2, 2)],[content substringWithRange:NSMakeRange(4, 2)],[content substringWithRange:NSMakeRange(6, 2)],[content substringWithRange:NSMakeRange(8, 2)],[content substringWithRange:NSMakeRange(10, 2)]];
        resultDic = @{@"macAddress":[macAddress uppercaseString]};
        operationID = mk_cl_taskReadDeviceWifiSTAMacAddressOperation;
        ...
        operationID = mk_cl_taskReadNTPServerHostOperation;
        ...
        operationID = mk_cl_taskReadServerHostOperation;
        ...
        resultDic = @{@"port":port};
        operationID = mk_cl_taskReadServerPortOperation;
        ...
        operationID = mk_cl_taskReadServerKeepAliveOperation;
        ...
        resultDic = @{@"qos":qos};
        operationID = mk_cl_taskReadServerQosOperation;
        ...
        operationID = mk_cl_taskReadSubscibeTopicOperation;
        ...
        operationID = mk_cl_taskReadPublishTopicOperation;
    }else if ([cmd isEqualToString:@"2a"]) {
        //读取LWT 开关
        BOOL isOn = ([content isEqualToString:@"01"]);
        resultDic = @{@"isOn":@(isOn)};
        operationID = mk_cl_taskReadLWTStatusOperation;
    }else if ([cmd isEqualToString:@"2b"]) {
        //读取LWT Qos
        NSString *qos = [MKBLEBaseSDKAdopter getDecimalStringWithHex:content range:NSMakeRange(0, content.length)];
        resultDic = @{@"qos":qos};
        operationID = mk_cl_taskReadLWTQosOperation;
    }else if ([cmd isEqualToString:@"2c"]) {
        //读取LWT Retain
        BOOL isOn = ([content isEqualToString:@"01"]);
        resultDic = @{@"isOn":@(isOn)};
        operationID = mk_cl_taskReadLWTRetainOperation;
        ...
        operationID = mk_cl_taskReadLWTTopicOperation;
        ...
        operationID = mk_cl_taskReadLWTPayloadOperation;
        ...
        resultDic = @{@"mode":mode};
        operationID = mk_cl_taskReadConnectModeOperation;
        ...
        operationID = mk_cl_taskReadWIFISSIDOperation;
        ...
        operationID = mk_cl_taskReadWIFIPasswordOperation;
    }else if ([cmd isEqualToString:@"42"]) {
        //读取DHCP开关
        BOOL isOn = ([content isEqualToString:@"01"]);
        resultDic = @{@"isOn":@(isOn)};
        operationID = mk_cl_taskReadDHCPStatusOperation;
    }else if ([cmd isEqualToString:@"43"]) {
        //读取IP信息
        NSString *ip = [MKCLSDKDataAdopter parseIpAddress:[content substringWithRange:NSMakeRange(0, 8)]];
        NSString *mask = [MKCLSDKDataAdopter parseIpAddress:[content substringWithRange:NSMakeRange(8, 8)]];
        NSString *gateway = [MKCLSDKDataAdopter parseIpAddress:[content substringWithRange:NSMakeRange(16, 8)]];
        NSString *dns = [MKCLSDKDataAdopter parseIpAddress:[content substringWithRange:NSMakeRange(24, 8)]];
        resultDic = @{
            @"ip":ip,
            @"mask":mask,
            @"gateway":gateway,
        ...
        operationID = mk_cl_taskReadNetworkIpInfosOperation;
        ...
        operationID = mk_cl_taskReadCountryBandOperation;
        ...
        operationID = mk_cl_taskReadRssiFilterValueOperation;
        ...
        operationID = mk_cl_taskReadFilterRelationshipOperation;
    }else if ([cmd isEqualToString:@"64"]) {
        //读取MAC过滤列表
        NSArray *macList = [MKCLSDKDataAdopter parseFilterMacList:content];
        resultDic = @{
            @"macList":(MKValidArray(macList) ? macList : @[]),
        };
        operationID = mk_cl_taskReadFilterMACAddressListOperation;
    }else if ([cmd isEqualToString:@"70"]) {
        //读取iBeacon开关
        BOOL isOn = ([content isEqualToString:@"01"]);
        resultDic = @{@"isOn":@(isOn)};
        operationID = mk_cl_taskReadAdvertiseBeaconStatusOperation;
        ...
        operationID = mk_cl_taskReadBeaconMajorOperation;
        ...
        operationID = mk_cl_taskReadBeaconMinorOperation;
    }else if ([cmd isEqualToString:@"73"]) {
        //读取iBeacon UUID
        resultDic = @{@"uuid":content};
        operationID = mk_cl_taskReadBeaconUUIDOperation;
        ...
        operationID = mk_cl_taskReadBeaconAdvIntervalOperation;
        ...
        operationID = mk_cl_taskReadBeaconTxPowerOperation;
    }else if ([cmd isEqualToString:@"80"]) {
        //读取计量数据上报开关
        BOOL isOn = ([content isEqualToString:@"01"]);
        resultDic = @{@"isOn":@(isOn)};
        operationID = mk_cl_taskReadMeteringSwitchOperation;
    }else if ([cmd isEqualToString:@"81"]) {
        //读取电量数据上报间隔
        NSString *interval = [MKBLEBaseSDKAdopter getDecimalStringWithHex:content range:NSMakeRange(0, content.length)];
        resultDic = @{@"interval":interval};
        operationID = mk_cl_taskReadPowerReportIntervalOperation;
    }else if ([cmd isEqualToString:@"82"]) {
        //读取电能数据上报间隔
        NSString *interval = [MKBLEBaseSDKAdopter getDecimalStringWithHex:content range:NSMakeRange(0, content.length)];
        resultDic = @{@"interval":interval};
        operationID = mk_cl_taskReadEnergyReportIntervalOperation;
    }else if ([cmd isEqualToString:@"83"]) {
        //读取负载检测通知开关
        BOOL isOn = ([content isEqualToString:@"01"]);
        resultDic = @{@"isOn":@(isOn)};
        operationID = mk_cl_taskReadLoadDetectionNotificationStatusOperation;
    }
...
+ (NSDictionary *)parseCustomConfigData:(NSString *)content cmd:(NSString *)cmd {
    ...
    if ([cmd isEqualToString:@"02"]) {
        //重启进入STA模式
        operationID = mk_cl_taskEnterSTAModeOperation;
    }else if ([cmd isEqualToString:@"11"]) {
        //配置NTP服务器域名
        operationID = mk_cl_taskConfigNTPServerHostOperation;
    }else if ([cmd isEqualToString:@"12"]) {
        //配置时区
        operationID = mk_cl_taskConfigTimeZoneOperation;
    }else if ([cmd isEqualToString:@"20"]) {
        //配置MQTT服务器域名
        operationID = mk_cl_taskConfigServerHostOperation;
    }else if ([cmd isEqualToString:@"21"]) {
        //配置MQTT服务器端口
        operationID = mk_cl_taskConfigServerPortOperation;
    }else if ([cmd isEqualToString:@"22"]) {
        //配置ClientID
        operationID = mk_cl_taskConfigClientIDOperation;
    }else if ([cmd isEqualToString:@"25"]) {
        //配置MQTT Clean Session
        operationID = mk_cl_taskConfigServerCleanSessionOperation;
    }else if ([cmd isEqualToString:@"26"]) {
        //配置MQTT KeepAlive
        operationID = mk_cl_taskConfigServerKeepAliveOperation;
    }else if ([cmd isEqualToString:@"27"]) {
        //配置MQTT Qos
        operationID = mk_cl_taskConfigServerQosOperation;
    }else if ([cmd isEqualToString:@"28"]) {
        //配置Subscribe topic
        operationID = mk_cl_taskConfigSubscibeTopicOperation;
    }else if ([cmd isEqualToString:@"29"]) {
        //配置Publish topic
        operationID = mk_cl_taskConfigPublishTopicOperation;
    }else if ([cmd isEqualToString:@"2a"]) {
        //配置LWT 开关
        operationID = mk_cl_taskConfigLWTStatusOperation;
    }else if ([cmd isEqualToString:@"2b"]) {
        //配置LWT Qos
        operationID = mk_cl_taskConfigLWTQosOperation;
    }else if ([cmd isEqualToString:@"2c"]) {
        //配置LWT Retain
        operationID = mk_cl_taskConfigLWTRetainOperation;
    }else if ([cmd isEqualToString:@"2d"]) {
        //配置LWT topic
        operationID = mk_cl_taskConfigLWTTopicOperation;
    }else if ([cmd isEqualToString:@"2e"]) {
        //配置LWT payload
        operationID = mk_cl_taskConfigLWTPayloadOperation;
    }else if ([cmd isEqualToString:@"2f"]) {
        //配置MTQQ服务器通信加密方式
        operationID = mk_cl_taskConfigConnectModeOperation;
    }else if ([cmd isEqualToString:@"40"]) {
        //配置WIFI SSID
        operationID = mk_cl_taskConfigWIFISSIDOperation;
    }else if ([cmd isEqualToString:@"41"]) {
        //配置WIFI password
        operationID = mk_cl_taskConfigWIFIPasswordOperation;
    }else if ([cmd isEqualToString:@"42"]) {
        //配置DHCP状态
        operationID = mk_cl_taskConfigDHCPStatusOperation;
    }else if ([cmd isEqualToString:@"43"]) {
        //配置IP地址相关信息
        operationID = mk_cl_taskConfigIpInfoOperation;
    }else if ([cmd isEqualToString:@"44"]) {
        //配置国家地区参数
        operationID = mk_cl_taskConfigCountryBandOperation;
    }else if ([cmd isEqualToString:@"60"]) {
        //配置扫描RSSI过滤
        operationID = mk_cl_taskConfigRssiFilterValueOperation;
    }else if ([cmd isEqualToString:@"61"]) {
        //配置扫描过滤逻辑
        operationID = mk_cl_taskConfigFilterRelationshipOperation;
    }else if ([cmd isEqualToString:@"64"]) {
        //配置MAC过滤规则
        operationID = mk_cl_taskConfigFilterMACAddressListOperation;
    }else if ([cmd isEqualToString:@"70"]) {
        //配置iBeacon 开关
        operationID = mk_cl_taskConfigAdvertiseBeaconStatusOperation;
    }else if ([cmd isEqualToString:@"71"]) {
        //配置iBeacon major
        operationID = mk_cl_taskConfigBeaconMajorOperation;
    }else if ([cmd isEqualToString:@"72"]) {
        //配置iBeacon minor
        operationID = mk_cl_taskConfigBeaconMinorOperation;
    }else if ([cmd isEqualToString:@"73"]) {
        //配置iBeacon uuid
        operationID = mk_cl_taskConfigBeaconUUIDOperation;
    }else if ([cmd isEqualToString:@"74"]) {
        //配置广播频率
        operationID = mk_cl_taskConfigAdvIntervalOperation;
    }else if ([cmd isEqualToString:@"75"]) {
        //配置发射功率
        operationID = mk_cl_taskConfigTxPowerOperation;
    }else if ([cmd isEqualToString:@"80"]) {
        //配置计量数据上报开关
        operationID = mk_cl_taskConfigMeteringSwitchOperation;
    }else if ([cmd isEqualToString:@"81"]) {
        //配置电量数据上报间隔
        operationID = mk_cl_taskConfigPowerReportIntervalOperation;
    }else if ([cmd isEqualToString:@"82"]) {
        //配置电能数据上报间隔
        operationID = mk_cl_taskConfigEnergyReportIntervalOperation;
    }else if ([cmd isEqualToString:@"83"]) {
        //配置负载检测通知开关
        operationID = mk_cl_taskConfigLoadDetectionNotificationStatusOperation;
    }
...
+ (NSDictionary *)parseMultiPackageReadData:(NSData *)readData cmd:(NSString *)cmd {
    ...
    NSDictionary *resultDic= @{
        mk_cl_totalNumKey:totalNum,
        mk_cl_totalIndexKey:index,
        mk_cl_contentKey:(subData ? subData : [NSData data]),
    };
    if ([cmd isEqualToString:@"23"]) {
        //读取服务器登录用户名
        operationID = mk_cl_taskReadServerUserNameOperation;
    }else if ([cmd isEqualToString:@"24"]) {
        //读取服务器登录密码
        operationID = mk_cl_taskReadServerPasswordOperation;
    }else if ([cmd isEqualToString:@"67"]) {
        //读取BLE Name过滤规则
        operationID = mk_cl_taskReadFilterAdvNameListOperation;
    }
    return [self dataParserGetDataSuccess:resultDic operationID:operationID];
}
...
+ (NSDictionary *)parseMultiPackageData:(NSString *)content cmd:(NSString *)cmd {
    ...
    if ([cmd isEqualToString:@"23"]) {
        //配置服务器登录用户名
        operationID = mk_cl_taskConfigServerUserNameOperation;
    }else if ([cmd isEqualToString:@"24"]) {
        //配置服务器登录密码
        operationID = mk_cl_taskConfigServerPasswordOperation;
    }else if ([cmd isEqualToString:@"30"]) {
        //配置CA file
        operationID = mk_cl_taskConfigCAFileOperation;
    }else if ([cmd isEqualToString:@"31"]) {
        //配置Client certificate file
        operationID = mk_cl_taskConfigClientCertOperation;
    }else if ([cmd isEqualToString:@"32"]) {
        //配置Client key file
        operationID = mk_cl_taskConfigClientPrivateKeyOperation;
    }else if ([cmd isEqualToString:@"67"]) {
        //配置Adv Name过滤规则
        operationID = mk_cl_taskConfigFilterAdvNameListOperation;
    }
    return [self dataParserGetDataSuccess:@{@"success":@(success)} operationID:operationID];
}

+ (NSDictionary *)dataParserGetDataSuccess:(NSDictionary *)returnData operationID:(mk_cl_taskOperationID)operationID{
    if (!returnData) {
        return @{};
    }
