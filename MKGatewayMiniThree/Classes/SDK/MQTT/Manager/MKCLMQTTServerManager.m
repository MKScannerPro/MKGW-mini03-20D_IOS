...
static MKCLMQTTServerManager *manager = nil;
static dispatch_once_t onceToken;

@interface NSObject (MKCLMQTTServerManager)

@end

@implementation NSObject (MKCLMQTTServerManager)

//+ (void)load{
//    [MKCLMQTTServerManager shared];
//}

@end

@interface MKCLMQTTServerManager ()

@property (nonatomic, assign)MKCLMQTTSessionManagerState state;

@property (nonatomic, strong)MKCLServerParamsModel *paramsModel;

@property (nonatomic, strong)NSMutableArray <id <MKCLServerManagerProtocol>>*managerList;

@property (nonatomic, strong)NSMutableDictionary *subscriptions;

@end

@implementation MKCLMQTTServerManager
...
+ (MKCLMQTTServerManager *)shared {
    dispatch_once(&onceToken, ^{
        if (!manager) {
            manager = [MKCLMQTTServerManager new];
        }
    });
    return manager;
}
...
- (void)sessionManager:(MQTTSessionManager *)sessionManager
    ...
    @synchronized (self.managerList) {
        for (id <MKCLServerManagerProtocol>protocol in self.managerList) {
            if ([protocol respondsToSelector:@selector(cl_didReceiveMessage:onTopic:)]) {
                [protocol cl_didReceiveMessage:dataDic onTopic:topic];
            }
        }
    }
}
...
- (void)sessionManager:(MQTTSessionManager *)sessionManager didChangeState:(MKMQTTSessionManagerState)newState {
    ...
    @synchronized (self.managerList) {
        for (id <MKCLServerManagerProtocol>protocol in self.managerList) {
            if ([protocol respondsToSelector:@selector(cl_didChangeState:)]) {
                [protocol cl_didChangeState:self.state];
            }
        }
    }
}
...
- (void)networkStateChanged{
    ...
    if (![[MKNetworkManager sharedInstance] currentNetworkAvailable]) {
        ...
        [self sessionManager:manager didChangeState:MKCLMQTTSessionManagerStateStarting];
        return;
    }
...
- (void)loadDataManager:(nonnull id <MKCLServerManagerProtocol>)dataManager {
    @synchronized (self.managerList) {
        if (dataManager
            && [dataManager conformsToProtocol:@protocol(MKCLServerManagerProtocol)]
...
- (BOOL)removeDataManager:(nonnull id <MKCLServerManagerProtocol>)dataManager {
    @synchronized (self.managerList) {
        if (!dataManager ||
            ![dataManager conformsToProtocol:@protocol(MKCLServerManagerProtocol)] ||
            ![self.managerList containsObject:dataManager]) {
...
- (void)loadLogFile {
    ...
    NSString *recordDateString = [[NSUserDefaults standardUserDefaults] objectForKey:@"mk_cl_mqtt_recordDate"];
    if (!ValidStr(recordDateString)) {
        //如果第一次存，则把当天的日期写入
        [[NSUserDefaults standardUserDefaults] setObject:[dateFormatter stringFromDate:[NSDate date]] forKey:@"mk_cl_mqtt_recordDate"];
        return;
    }
    //写入当天日期，用来做比较
    [[NSUserDefaults standardUserDefaults] setObject:[dateFormatter stringFromDate:[NSDate date]] forKey:@"mk_cl_mqtt_recordDate"];
...
#pragma mark - getter
- (MKCLServerParamsModel *)paramsModel {
    if (!_paramsModel) {
        _paramsModel = [[MKCLServerParamsModel alloc] init];
    }
    return _paramsModel;
}

- (NSMutableArray<id<MKCLServerManagerProtocol>> *)managerList {
    if (!_managerList) {
