...
/**
 线程ID
 */
@property (nonatomic, assign)mk_cl_serverOperationID operationID;
...
@implementation MKCLMQTTOperation
@synthesize executing = _executing;
@synthesize finished = _finished;

- (void)dealloc{
    NSLog(@"MKCLMQTTOperation销毁");
}

- (instancetype)initOperationWithID:(mk_cl_serverOperationID)operationID
...
#pragma mark - MKCLMQTTOperationProtocol
...
- (void)didReceiveMessage:(NSDictionary *)data onTopic:(NSString *)topic {
    ...
    [self dataParserReceivedData:[MKCLMQTTTaskAdopter parseDataWithJson:data topic:topic]];
}
...
- (void)dataParserReceivedData:(NSDictionary *)dataDic{
    ...
    mk_cl_serverOperationID operationID = [dataDic[@"operationID"] integerValue];
    if (operationID == mk_cl_defaultServerOperationID || operationID != self.operationID) {
        return;
    }
