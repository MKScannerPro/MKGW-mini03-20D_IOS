...
@implementation MKCLCreatScannerProDeviceModel
...
@interface MKCLNetworkService ()

@property (nonatomic, strong)NSURLSessionDataTask *addScannerProDeviceTask;

@end

@implementation MKCLNetworkService

+ (instancetype)share {
    static dispatch_once_t t;
    static MKCLNetworkService *service = nil;
    dispatch_once(&t, ^{
        service = [[MKCLNetworkService alloc] init];
    });
    return service;
}

- (void)addScannerProDevicesToCloud:(NSArray <MKCLCreatScannerProDeviceModel *>*)deviceList
                             isHome:(BOOL)isHome
    ...
    for (MKCLCreatScannerProDeviceModel *deviceModel in deviceList) {
