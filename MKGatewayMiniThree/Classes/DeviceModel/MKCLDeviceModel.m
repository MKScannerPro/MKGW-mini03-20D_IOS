...
NSString *const MKCLDeviceModelOfflineNotification = @"MKCLDeviceModelOfflineNotification";

@interface MKCLDeviceModel ()
...
@implementation MKCLDeviceModel

- (void)dealloc{
    NSLog(@"MKCLDeviceModel销毁");
}

#pragma mark - public method

- (void)startStateMonitoringTimer{
    if (self.onLineState == MKCLDeviceModelStateOffline) {
        return;
    }
    ...
    dispatch_source_set_event_handler(self.receiveTimer, ^{
        ...
        if (self.receiveTimerCount >= 62.f) {
            ...
            self.receiveTimerCount = 0;
            self.onLineState = MKCLDeviceModelStateOffline;
            dispatch_async(dispatch_get_main_queue(), ^{
                [[NSNotificationCenter defaultCenter] postNotificationName:MKCLDeviceModelOfflineNotification
                                                                    object:nil
                                                                  userInfo:@{@"macAddress":self.macAddress}];
                if ([self.delegate respondsToSelector:@selector(cl_deviceOfflineWithMacAddress:)]) {
                    [self.delegate cl_deviceOfflineWithMacAddress:self.macAddress];
                }
            });
            return ;
        }
...
- (NSString *)currentSubscribedTopic {
    if (ValidStr([MKCLMQTTDataManager shared].serverParams.publishTopic)) {
        return [MKCLMQTTDataManager shared].serverParams.publishTopic;
    }
...
- (NSString *)currentPublishedTopic {
    if (ValidStr([MKCLMQTTDataManager shared].serverParams.subscribeTopic)) {
        return [MKCLMQTTDataManager shared].serverParams.subscribeTopic;
    }
