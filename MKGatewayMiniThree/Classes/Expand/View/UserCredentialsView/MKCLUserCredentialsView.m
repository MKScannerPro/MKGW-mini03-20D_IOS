...
@implementation MKCLUserCredentialsViewModel
@end

@interface MKCLUserCredentialsView ()
...
@implementation MKCLUserCredentialsView
...
#pragma mark - setter
- (void)setDataModel:(MKCLUserCredentialsViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCLUserCredentialsViewModel.class]) {
        return;
    }
...
- (MKTextField *)userNameTextField {
    if (!_userNameTextField) {
        ...
        _userNameTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(cl_mqtt_userCredentials_userNameChanged:)]) {
                [self.delegate cl_mqtt_userCredentials_userNameChanged:text];
            }
        };
...
- (MKTextField *)passwordTextField {
    if (!_passwordTextField) {
        ...
        _passwordTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(cl_mqtt_userCredentials_passwordChanged:)]) {
                [self.delegate cl_mqtt_userCredentials_passwordChanged:text];
            }
        };
