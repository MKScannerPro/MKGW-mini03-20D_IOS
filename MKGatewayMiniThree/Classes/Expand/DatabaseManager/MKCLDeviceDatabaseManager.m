...
@implementation MKCLDeviceDatabaseManager

+ (BOOL)initDataBase {
    FMDatabase* db = [FMDatabase databaseWithPath:kFilePath(@"CLDeviceDB")];
    if (![db open]) {
        return NO;
    }
    NSString *sqlCreateTable = [NSString stringWithFormat:@"create table if not exists CLDeviceTable (deviceType text,clientID text,deviceName text,subscribedTopic text,publishedTopic text,macAddress text, lwtStatus text,lwtTopic text)"];
...
+ (void)insertDeviceList:(NSArray <MKCLDeviceModel *>*)deviceList
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"CLDeviceDB")] inDatabase:^(FMDatabase *db) {
        
        for (MKCLDeviceModel *device in deviceList) {
            BOOL exist = NO;
            FMResultSet * result = [db executeQuery:@"select * from CLDeviceTable where macAddress = ?",device.macAddress];
            while (result.next) {
...
+ (void)deleteDeviceWithMacAddress:(NSString *)macAddress
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"CLDeviceDB")] inDatabase:^(FMDatabase *db) {
        ...
        BOOL result = [db executeUpdate:@"DELETE FROM CLDeviceTable WHERE macAddress = ?",macAddress];
        if (!result) {
            [self operationDeleteFailedBlock:failedBlock];
            return;
        }
...
+ (void)readLocalDeviceWithSucBlock:(void (^)(NSArray <MKCLDeviceModel *> *deviceList))sucBlock
                        failedBlock:(void (^)(NSError *error))failedBlock {
    FMDatabase* db = [FMDatabase databaseWithPath:kFilePath(@"CLDeviceDB")];
    if (![db open]) {
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"CLDeviceDB")] inDatabase:^(FMDatabase *db) {
        ...
        FMResultSet * result = [db executeQuery:@"SELECT * FROM CLDeviceTable"];
        while ([result next]) {
            MKCLDeviceModel *dataModel = [[MKCLDeviceModel alloc] init];
...
+ (void)updateLocalName:(NSString *)localName
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"CLDeviceDB")] inDatabase:^(FMDatabase *db) {
        ...
        BOOL exist = NO;
        FMResultSet * result = [db executeQuery:@"select * from CLDeviceTable where macAddress = ?",macAddress];
        while (result.next) {
        ...
        //存在该设备，更新设备
        [db executeUpdate:@"UPDATE CLDeviceTable SET deviceName = ? WHERE macAddress = ?",localName,macAddress];
        if (sucBlock) {
            moko_dispatch_main_safe(^{
                sucBlock();
            });
...
+ (void)updateClientID:(NSString *)clientID
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"CLDeviceDB")] inDatabase:^(FMDatabase *db) {
        ...
        //存在该设备，更新设备
        [db executeUpdate:@"UPDATE CLDeviceTable SET clientID = ?, subscribedTopic = ? ,publishedTopic = ? , lwtStatus = ?,lwtTopic = ? WHERE macAddress = ?",SafeStr(clientID),SafeStr(subscribedTopic),SafeStr(publishedTopic),(lwtStatus ? @"1" : @"0"),SafeStr(lwtTopic),SafeStr(macAddress)];
        if (sucBlock) {
            moko_dispatch_main_safe(^{
                sucBlock();
            });
